...
int main(int argc, const char **argv) {
...

    {
        ...
        XMLText *text = doc.FirstChildElement("document")->FirstChildElement(
                (const char *) russianElementName)->FirstChild()->ToText();
...

    {
        ...
        XMLElement *ele = doc.NewElement(
                "unused");        // This will get cleaned up with the 'doc' going out of scope.
...

    {
        ...
        doc.Parse(xml2);
        XMLTest("Test that declaration after self-closed child is not allowed", XML_ERROR_PARSING_DECLARATION,
                doc.ErrorID());
        doc.Parse(xml3);
